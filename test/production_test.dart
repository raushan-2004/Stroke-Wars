...
void main() {
  group(
    'Production Readiness, Performance & Release (Stage 12) — Integration Tests',
    () {
      ...
      test(
        'EnvironmentManager switches and loads configurations correctly',
        () {
          expect(
            environmentManager.currentEnv,
            equals(SWEnvironment.development),
          );
          expect(
            environmentManager.currentConfig.apiEndpoint,
            contains('dev.strokewars.local'),
          );
          ...
          expect(
            environmentManager.currentEnv,
            equals(SWEnvironment.production),
          );
          expect(
            environmentManager.currentConfig.apiEndpoint,
            equals('https://api.strokewars.com'),
          );
          expect(
            environmentManager.isFeatureEnabled('experimental_brush'),
            isFalse,
          );
        },
      );
      ...
      test(
        'LoggingService filters credentials, emails, and auth keys successfully',
        () {
      ...
      test('DiagnosticsCenter aggregates analytics and handles failures', () {
        ...
        expect(
          loggingService.logs.first.message,
          contains('Application initialized'),
        );

        final testError = StateError('Test panic');
        diagnosticsCenter.reportFailure(
          'Fatal runtime crash',
          testError,
          StackTrace.current,
        );
        ...
        expect(
          crashReportingService.reports.first['error'],
          contains('Test panic'),
        );
      });
      ...
      test('ReleaseValidator audits production settings, locales, and assets', () {
        ...
        expect(
          failReport.errors,
          hasLength(5),
        ); // https, dev domain, crash reporting disabled, missing fallback locale, empty assets
      });

      test(
        'SecurityManager abstraction verification hashes and secure stores',
        () {
          ...
          final expectedHash = sha256
              .convert(utf8.encode(rawPayload))
              .toString();
          expect(
            securityManager.verifyReplayIntegrity(rawPayload, expectedHash),
            isTrue,
          );
      ...
      test(
        'LocalizationManager handles Spanish conversions and fallback behaviors',
        () {
          // Default: English
          expect(
            localizationManager.translate('app_title'),
            equals('Stroke Wars'),
          );

          // Plurals conversion
          expect(
            localizationManager.translatePlural('many_players', 1),
            equals('1 Player'),
          );
          expect(
            localizationManager.translatePlural('many_players', 5),
            equals('5 Players'),
          );
          ...
          expect(
            localizationManager.translate('lobby_title'),
            equals('Sala LAN'),
          ); // Translated in Spanish
          localizationManager.switchLocale('es');
          expect(
            localizationManager.translate('missing_translated_key'),
            equals('missing_translated_key'),
          );
        },
      );
    },
