...
class MockTransport implements GameTransport {
  ...
  Future<void> send(String peerId, String message) async {
    peer?._messageController.add(
      TransportMessage(senderId: isHost ? 'host' : peerId, content: message),
    );
  }
...
void main() {
  group('Multiplayer Stage 6A — Unit & Integration Tests', () {
    ...
    test('ProtocolSerializer rejects invalid versions', () {
      ...
      final validationResult = serializer.validateVersions(
    ...
    test(
      'ConnectionManager tracks heartbeats, timeouts and sequence filters duplicates',
      () async {
        ...
        // Send payload
        await clientManager.sendPayload(
          const VersionMessage(protocolVersion: 1, engineVersion: '1.0.0'),
        );

        final receivedEnvelope = await receivedFuture;
        expect(
          receivedEnvelope.connectionId,
          const ConnectionId('client-conn'),
        );
        ...
        hostTransport._messageController.add(
          TransportMessage(
        ...
        expect(
          hostManager.statistics.packetsDropped,
          greaterThanOrEqualTo(1),
        ); // Packets dropped count incremented
    ...
    test(
      'HostController and ClientController align and synchronize matches authoritative events',
      () async {
        // 1. Setup Controllers
        final hostTransport = MockTransport(isHost: true);
        final clientTransport = MockTransport(isHost: false);
    ...
    test('PeerDiscoveryService advertises and scans valid rooms', () async {
      final discovery = PeerDiscoveryService(discoveryPort: 18085);
    ...
    test(
      'LocalNetworkTransport connects, disconnects, and broadcasts real TCP sockets',
      () async {
