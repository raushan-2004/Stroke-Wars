...
void main() {
  group('LAN Gameplay Integration (Stage 6B) — Unit & Integration Tests', () {
    ...
    test(
      'SynchronizationManager sequence validation, duplicate filters and out-of-order detection',
      () {
    ...
    test(
      'SynchronizationManager rejects unauthorised non-drawer Canvas events host-side',
      () {
        final manager = SynchronizationManager(
          matchController: matchController,
          canvasController: canvasController,
    ...
    test(
      'LANSessionCoordinator updates lifecycle states during Host and Join runs',
      () async {
        ...
        expect(
          coordinator.session.sessionState,
          LANSessionLifecycleState.discovering,
        );
        ...
        expect(
          coordinator.session.sessionState,
          LANSessionLifecycleState.lobby,
        );
        ...
        expect(
          coordinator.session.sessionState,
          LANSessionLifecycleState.closed,
        );
        coordinator.dispose();
      },
    );
